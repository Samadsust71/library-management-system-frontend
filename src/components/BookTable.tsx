...
const BookTable=({ books, onBorrowClick }: BookTableProps) =>{
  ...
  const getAvailabilityBadge = (book: DBBook) => {
    return book.copies > 0 ? (
      <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium w-fit bg-green-100 text-green-600">
        Available
      </span>
    ) : (
      <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-400 w-fit">
        Unavailable
      </span>
    );
  };
              ...
              <TableRow key={book._id} className="hover:bg-muted/50">
                ...
                <TableCell>
                  <div className="flex flex-col gap-1">
                    ...
                    <span className="text-xs text-muted-foreground pl-2.5">
                      {book.copies} copies
                    </span>
                  </div>
                </TableCell>
